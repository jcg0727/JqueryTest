...
<body>
	<table border="1">
		<tr>
			<td colspan="4"><a href="<%=request.getContextPath() %>insert.do">[회원등록]</a></td> <!-- 절대경로. 상대경로로 쓰면 아무것도 안쓰고 파일명만 -->
		</tr>
		<tr>
			<td>ID</td>
			<td>이름</td>
...
%>
			<tr>
				...
				<td><a href="<%=request.getContextPath()%>/member/select.do?memId=<%= memList.get(i).getMemId()%>"><%=memList.get(i).getMemName() %></a></td>
	...
	<%if(pagingVO.getTotalCount() > 0) { %>
		<tr>
			<td colspan="4" align="center">
				...
				<a href="list.do?pageNo=<%=pagingVO.getFirstPageNo()-pagingVO.getPageCount() %>">[이전]</a>
				<%} %>
				<%for(int pNo=pagingVO.getFirstPageNo(); pNo<=pagingVO.getLastPageNo();pNo++){ %>
					<a href="list.do?pageNo=<%=pNo %>">[<%=pNo %>]</a>
				<%} %>
				<%if(pagingVO.getLastPageNo() < pagingVO.getTotalPageCount()){ %>
					<a href="list.do?pageNo=<%=pagingVO.getFirstPageNo()+pagingVO.getPageCount()%>">[다음]</a>
				<%} %>
			</td>
		</tr>
	<%}%>
